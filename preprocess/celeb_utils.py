...
def filter_images_by_attribute(data_dir, attr_dir, attr1=None, attr2=None, present1=True, present2=True):
    if attr1 is None and attr2 is None:
        return glob(os.path.join(data_dir, 'img_align_celeba/*.jpg'))
    df = pd.read_csv(os.path.join(attr_dir, 'list_attr_celeba.csv'))
    ...
    image_ids = [data_dir+'/img_align_celeba/' + i for i in image_ids]
...
def show_images(ids):
    show_n_images = 16
    celeb_images = get_batch(ids[:show_n_images], 28, 28, 'RGB')
...
def plot_sweeps_celeb(model, latent_dim=2):
    n = 10
    digit_size = 28
    n_channels = 3
    figure = np.zeros((digit_size * n, digit_size * n, n_channels))
...
def get_synthetic_images(model, latent_dim=2, n=16):
    ...
    print("Device tensor is stored on: ", h2.device)

    return model.decode(h2)


# def extract_bg_salient(model,original_img):

#     recon_img test_reconstruction(original_img)

#     bg_img, _, _, _, _ = model.forward_background(original_img)

#     s_img, _, _, _, _ = model.forward_target(original_img)